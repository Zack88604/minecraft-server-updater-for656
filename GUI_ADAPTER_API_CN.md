...
服务端发布的预设使用与上文完全相同的 V1 或 V2 JAR 元数据。更新服务器只分发
描述和归档，不会获得额外的 GUI 控制 API。服务端预设策略见 README。

更新器会在 `.mc-update/gui-presets/` 中管理名为 `server-<id>.jar` 的归档；
手动安装的预设不要使用该保留名称。用户只需对某个 `服务器 URL + id` 批准一次；
同一身份的新版本可以直接加载。服务器 URL 或 id 改变时会重新回到可信 Swing
确认对话框。
...
2. **已保存 Swing 选择** → 直接启动。
3. **已保存外部预设** → 直接启动。仅在用户首次选择该外部预设时显示**风险确认**。
4. **回退**：用户拒绝首次确认、加载失败或元数据消失 → 回退到内建 Swing（加载失败还会
   清除已保存的选择）。
5. 删除 `gui-selection.properties` → 重新显示选择窗口。

#### 安全说明

- 用户首次选择外部预设并确认风险后才会加载类；之后会按已保存的选择直接加载。
...
V2 预设。用户首次选择预设并接受外部代码风险提示后，更新器才会校验并解压运行时产物，
然后启动独立的 Java 子进程。子进程只接收完整的、最新的 `UpdateUiState` 快照，并且只能传回
标准的关闭动作。
